...
exports.userCtrl = {
  ...
  addScreen: async (req, res) => {
    try {
      const { userId, nameScreen, imageScreen, favs_ar, lenguage } = req.body;
      ...
      const newScreen = {
        ...
        favs_ar: favs_ar || [],
        lenguage: lenguage?.toLowerCase() || "en",
      };
  ...
  editScreen: async (req, res) => {
    try {
      ...
      screen.lenguage = lenguage?.toLowerCase() || "en";
  ...
  deleteScreen: async (req, res) => {
    try {
      ...
      if (screenIndex === 0) {
        return res.status(400).json({ error: "Cnot delete profile account" });
      }
